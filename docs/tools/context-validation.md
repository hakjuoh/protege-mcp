...
- `external_mappings`: `{available: false, reason: "mapping management is not available in this release"}` — SSSOM mappings are managed by the dedicated mapping tools, but this impact analysis does not traverse the mapping sidecar.
