...
2. **Register it.** Make sure your `*Tools.specs(ctx)` is added in
   [`ToolCatalog.buildAll`](https://github.com/hakjuoh/protege-mcp/blob/main/src/main/java/io/github/hakjuoh/tools/ToolCatalog.java).
3. **Test it.** Add a unit test for the core, and extend `ToolPipelineTest` if it participates in the
   end-to-end flow.
4. **Document it.** Add the tool to the README table **and** to the matching page under
   [Tools](tools/) (arguments + returns).
