...
- Semantic diff, impact analysis, release manifest, and validation reports.
...
| --- | --- | --- |
| 0. Verified exchange | Versioned RO-Crate, RDFC identity, locked imports, full offline QC, verified release bundle, manual/vendor-tool import, exported snapshot diff | Available; target procedure and target-side read-back remain user-maintained |
...
Connector implementation remains roadmap work under commercial-platform interoperability. The release
manifest and headless workspace prerequisites now exist; future adapters must consume those verified
artifacts rather than bypass them, and vendor SDKs and credentials must remain outside the
