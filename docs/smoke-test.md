...
| 65 | Save the active ontology, run `write_project_policy_template` on the fresh project, then `validate_project_policy` | the writer uses that saved document as `root_artifact`, creates matching minimal RO-Crate metadata, selects a unique installed reasoner only when possible, and returns an immediately valid policy without follow-up file searches or rewrites |
| 66 | Use `write_project_policy patch={...}` to change nested fields in `external_terms`, `network`, and `audit`; then submit an invalid patch | the valid recursive patch preserves omitted nested fields and all unaffected top-level comments/bytes; arrays/scalars replace and null removes a field; the invalid merged candidate returns `policy_invalid` and leaves the original file byte-identical |
| 66a | On a saved project with no policy, call `write_project_policy` once with a patch adding BioPortal, ArgoPortal, and EMBL-EBI provider rows | the tool internally scaffolds v3 plus matching RO-Crate metadata, applies the general patch, returns `created_from_template=true` and `valid=true`, and leaves no generated files if the combined candidate is invalid |
...
| 76 | Start `serve --transport stdio --project <policy>`, initialize it, and list/call tools | stdout contains only bounded JSON-RPC; the eighteen supported project tools work offline and every GUI-only tool is listed explicitly as unavailable |
...
| 91 | Call `get_reasoner_capabilities` and `validate_rules` through live and headless adapters, including an ontology with unsupported SWRL atoms/built-ins | both surfaces return the same exact-profile vocabulary; validation parses every rule without executing it, and `run_project_qc` includes the automatic `rules` stage |
| 92 | Preview all six materialization categories into a new live ontology, verify the source is unchanged, then commit; repeat against `active_source` with `allow_source=true` and invoke Undo once | preview changes no live state; the new ontology is created through the model manager and reports `single_undo=false`; the active-source axiom batch is reverted by one Undo |
| 93 | Through headless stdio, preview into a new project file, commit with confirmation, repeat the same artifact, then exercise merge/replace collisions and an existing file above `max_bytes` | the first commit uses verified serialization, anchored staging, backup, and guarded no-overwrite publication; the repeat is `noop`; collisions follow their explicit mode; the oversized target returns `materialization_bound_exceeded` without reading past the bound |
| 94 | Force timeout, blocked cleanup, cleanup-executor rejection, source/policy/reasoner drift, backup-only publication, and post-replacement verification failure | no incomplete preview is published; later previews stay fenced while private cleanup is unresolved; prevented failures say so, known partial effects include a fact-by-fact receipt, and uncertain applied mutations require a state check before retry |
| 95 | In **Settings ▸ Ontology Assistant ▸ Available models**, add, reorder, and delete ids for an installed CLI, reopen the Ontology Assistant, then send one short turn with a chosen model and reasoning effort and one with both pickers on `(default)` | the picker offers `(default)` plus exactly the saved catalog in its saved order; the chosen turn runs at that model/effort; the `(default)` turn passes no model or effort flag and uses the CLI's own configuration |
| 96 | With the Ontology Assistant open, add one id with **Enter**, select a row and rename it with **Enter**, press **Enter** on an empty field, then delete the id the chat currently has selected and press **OK** | Enter adds and updates like the button; on an empty field it activates the dialog's OK instead; every staged edit says it is not saved until OK; the open Assistant's picker shows the new list without being reopened, and the deleted selection falls back to `(default)` |
| 97 | Pick a reasoning effort the selected model does not support (Codex), and — on a `claude` release that only warns about unknown `--effort` values — pick one it rejects | Codex fails the turn with the API's supported-values error; `claude` still answers; in both cases a note after the turn names the effort picker and how to get back to `(default)` |
| 98 | Configure the `ontoportal` profile with the BioPortal preset, AgroPortal preset, or a compatible custom HTTPS origin owner-locally, enable its alias in policy v2 without adding the host to global `network.allowed_hosts`, and ask the Ontology Assistant to search it; repeat after disabling **General ▸ Allow the Ontology Assistant to use project-approved terminology registries** | the enabled turn returns provider evidence through `search_external_terms`; its principal has `external-terms:read` and no general `network:access`; requests and cached evidence remain confined to the exact owner-bound origin; the disabled turn is denied before provider transport and sends no request |
