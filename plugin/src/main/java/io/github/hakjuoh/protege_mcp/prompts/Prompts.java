    ...
    public static void register(PromptRegistry prompts) {
        prompts.prompt("audit_ontology",
                ...
                args -> ""
                        ...
                        + "2. Call get_project_policy. If policy_loaded=true, call run_project_qc and "
                        + "treat gate=error as an incomplete audit (never as a policy failure or pass). "
                        + "If no policy is present, call validate_ontology and validate_governance, and "
                        + "state that this is the legacy interactive audit rather than a reproducible "
                        + "project gate.\n"
                        + "3. If project QC did not already run a current reasoner stage, call run_reasoner "
                        + "for logical problems. If it reports the ontology "
                        ...
                        + "preview_change_set to normalize the exact axioms and run isolated project QC "
                        + "without touching the live ontology or Undo stack. DO NOT modify anything until "
                        + "I approve. After approval, call commit_change_set with its change_set_id and, "
                        + "as expected_revision, its complete base_revision; report any revision_conflict "
                        + "instead of auto-merging. "
                        + "Use preview_changes then apply_changes with verify=\"rollback\" only as the compatibility "
        ...
        prompts.prompt("add_subclass_safely",
                ...
                args -> {
                    ...
                    return ""
                            ...
                            + "entity from a typo.\n"
                            + "2. Call preview_change_set with one operation {op:add, "
                            + "axiom_type:subclass_of, sub:\"" + child + "\", super:\"" + parent
                            + "\"}. Show its normalized delta, base_revision, preflight gate, and any new "
                            + "entities; a non-pass/error gate is not committable.\n"
                            + "3. After I approve, call commit_change_set with that change_set_id and, as "
                            + "expected_revision, the complete base_revision. Report committed, "
                            + "undo_logged, new_revision, or a "
                            + "revision_conflict; never rebuild or auto-merge a stale preview.\n"
                            + "4. If satisfiability_checked=false, inspect policy_loaded and the preflight "
                            + "reasoner row before retrying. With no policy, or when policy requires a "
                            + "reasoner but reports none/wrong selection, use list_reasoners/set_reasoner "
                            + "and create a fresh preview; fix a reported reasoner error first. If a loaded "
                            + "policy intentionally omits the reasoner stage, selecting a reasoner alone "
                            + "will not add it: disclose that the gate did not check satisfiability and ask "
                            + "whether to update the policy/include the stage or explicitly approve the "
                            + "reviewed change set and commit it as UNVERIFIED. Do the latter as well if no "
                            + "reasoner is available. Only when the change-set tools are unavailable, use "
                            + "preview_changes then apply_changes with verify=\"rollback\" as the legacy "
                            + "fallback, after the same UNVERIFIED disclosure when no verdict is possible.";
                });
        ...
        prompts.prompt("model_domain",
                ...
                args -> {
                    ...
                    return ""
                            ...
                            + "batch with preview=true and strict=true, plus preview_change_set for any "
                            + "remaining axioms. Each preview must show its complete base_revision and "
                            + "isolated preflight gate without mutating the ontology.\n"
                            + "4. After I approve, commit each reviewed preview with commit_change_set, "
                            + "passing its exact base_revision as expected_revision and refreshing later "
                            + "previews after each successful "
                            + "commit. Report conflicts instead of auto-merging. On an older server without "
                            + "change sets, use preview_changes for the remaining operations, the direct "
                            + "batch/apply_changes workflow, then get_project_policy/run_project_qc or "
                            + "run_qc_suite when no policy is loaded. Work in small, reviewable batches.";
                });
        ...
        prompts.prompt("refactor_entity_safely",
                ...
                args -> {
                    ...
                    return ""
                            ...
                            + "3. Prefer the change-set workflow for any exact axiom add/remove portion: "
                            + "preview_change_set, then commit_change_set with the complete base_revision "
                            + "as expected_revision. "
                            + "For the high-level legacy macros, rename_entity and delete_entity take "
                            + "preview=true — show me the "
                            ...
                            + "4. Apply/commit ONLY after I approve. Each is a single undo transaction — "
        ...
        prompts.prompt("release_readiness_check",
                ...
                args -> {
                    ...
                    return ""
                            ...
                            + "2. Call get_project_policy. If policy_loaded=true, call run_project_qc. "
                            + "A required stage that is skipped, errors, uses the wrong reasoner, or lacks "
                            + "its persisted asset must yield gate=error and a DO-NOT-SHIP verdict.\n"
                            + "3. Only when no policy exists, run the legacy checks: call run_reasoner, then "
                            + "run_qc_suite with stages=[\"reasoner\",\"profile\",\"governance\","
                            + "\"structural\",\"cqs\"] and owl_profile=\"" + profile + "\", "
                            + "required_stages set to the same list, and error_on_missing_required=true. "
                            + "Do not waive a missing CQ/SHACL/invariant stage merely because its backing "
                            + "data is absent.\n"
                            + "4. In that no-policy fallback, call validate_governance with owl_profile=\""
                            + profile + "\", "
