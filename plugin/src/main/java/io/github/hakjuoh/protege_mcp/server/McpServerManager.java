...
public final class McpServerManager {
    ...
    public static final String SERVER_VERSION = "0.7.2";
    ...
    private static final String INSTRUCTIONS =
            "Tools to read and edit the ontology currently open in Protégé Desktop. Every tool returns "
                    ...
                    + "from a typo).\n"
                    + "3. For reviewed edits, call preview_change_set with exact operations, inspect its "
                    + "isolated policy/QC gate, then pass its complete base_revision to commit_change_set "
                    + "as expected_revision. "
                    + "The final commit rechecks prefixes, policy, read-only mode and workspace state after "
                    + "confirmation, and applies at most one Undo entry. preview_changes/apply_changes remain "
                    + "the compatibility path for clients without change-set support and for user-directed "
                    + "direct edits.\n"
                    + "4. Verify: prefer run_project_qc when a project policy is loaded; otherwise call "
                    + "run_reasoner before inferred reads and validate_ontology for modelling quality.\n"
    ...
    public void build(List<SyncToolSpecification> tools) {
        ...
        this.transport = HttpServletStreamableServerTransportProvider.builder()
                ...
                .contextExtractor(request -> {
                    Object principal = request.getAttribute(AuthenticatedPrincipal.REQUEST_ATTRIBUTE);
                    return principal instanceof AuthenticatedPrincipal
                            ? io.modelcontextprotocol.common.McpTransportContext.create(
                                    java.util.Map.of(AuthenticatedPrincipal.CONTEXT_KEY, principal))
