...
> shipping**: 83 structured tools + 11 guided prompts (§5) behind one fixed MCP endpoint that a shared broker
...
## 5. MCP Tool Catalog (83 tools + 11 prompts)

Eighty-three tools — 7 read, 2 context, 1 revision (`get_model_revision`), 24
...
`explain_inconsistency`), 3 SPARQL (`sparql_query`/`sparql_schema`/`sparql_validate`), 6 validation/diff
(incl. `diff_ontologies`, `semantic_diff`, `analyze_change_impact`, `validate_governance`, `shacl_validate`), and 12
...
`validate_project_policy`, `run_project_qc`, `write_project_policy_template`, `run_release_gate`, `prepare_release`) — each defined by a `name`, a `description`, and a
