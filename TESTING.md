...
At the time of writing: **2,044 tests, green**, across `tools`, `oauth`, `server`, `chat`,
`config`, and the pure helpers of `ui`. Coverage is measured by **JaCoCo** (`mvn verify`) and a floor on
...
- **`Tools` (754 → 383 LOC)** → 7 focused classes in `io.github.hakjuoh.tools`: `ToolArgs`,
...
- **`ChatView`** → pure logic extracted to headless classes in `io.github.hakjuoh.chat`:
...
`src/test/java/io/github/hakjuoh/tools/FakeModelManager.java` is a `Proxy`-based test double for
...
It is package-private to `io.github.hakjuoh.tools`, so only tests in that package can use it.
